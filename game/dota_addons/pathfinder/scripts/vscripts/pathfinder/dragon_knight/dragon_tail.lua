...
-- LinkLuaModifier( "modifier_pathfinder_dk_dragon_tail", "pathfinder/dragon_knight/dragon_tail", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_dk_dragon_tail_attack", "pathfinder/dragon_knight/dragon_tail", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_dk_dragon_tail_passive", "pathfinder/dragon_knight/dragon_tail", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_dk_dragon_tail_bonus_shard_range", "pathfinder/dragon_knight/dragon_tail", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_dk_dragon_tail_chain", "pathfinder/dragon_knight/dragon_tail", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_dk_dragon_tail_chain_unkillable", "pathfinder/dragon_knight/dragon_tail", LUA_MODIFIER_MOTION_NONE )

function pathfinder_dk_dragon_tail:GetIntrinsicModifierName()
	return "modifier_pathfinder_dk_dragon_tail_passive"
end
...
function pathfinder_dk_dragon_tail:GetCastRange( vLocation, hTarget )
	if self:GetCaster():HasModifier( "modifier_pathfinder_dk_elder_dragon_form" ) then
		return self:GetSpecialValueFor("dragon_cast_range") + self:GetCaster():GetModifierStackCount("modifier_pathfinder_dk_dragon_tail_bonus_shard_range", self:GetCaster())
	else
		return self.BaseClass.GetCastRange( self, vLocation, hTarget ) + self:GetCaster():GetModifierStackCount("modifier_pathfinder_dk_dragon_tail_bonus_shard_range", self:GetCaster())
	end
end
...
function pathfinder_dk_dragon_tail:Spawn()
	if IsServer() then
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_pathfinder_dk_dragon_tail_bonus_shard_range", {})
	end
end
...
function pathfinder_dk_dragon_tail:OnSpellStart()
	...
	local modifier = caster:FindModifierByNameAndCaster( "modifier_pathfinder_dk_elder_dragon_form", caster )
...
function pathfinder_dk_dragon_tail:HitSingle( target )
	...
	caster:AddNewModifier(
		caster,
		self,
		"modifier_pathfinder_dk_dragon_tail_attack",
		{}
	)
	caster:PerformAttack(target, false, true, true, false, false, false, true)
	caster:RemoveModifierByName("modifier_pathfinder_dk_dragon_tail_attack")
...
function pathfinder_dk_dragon_tail:Hit( target, dragonform )
	...
	if caster:HasAbility("pathfinder_dk_dragon_tail_chain") and target and target:GetTeamNumber() ~= caster:GetTeamNumber() then
		local shard = caster:FindAbilityByName("pathfinder_dk_dragon_tail_chain")
		local mod_duration = shard:GetLevelSpecialValueFor("duration",1)
		target:AddNewModifier(caster, self, "modifier_pathfinder_dk_dragon_tail_chain", {duration = mod_duration})
	end
end
...
-- modifier_pathfinder_dk_dragon_tail = {}

-- function modifier_pathfinder_dk_dragon_tail:IsDebuff()
-- 	return true
-- end

-- function modifier_pathfinder_dk_dragon_tail:IsStunDebuff()
-- 	return true
-- end

-- function modifier_pathfinder_dk_dragon_tail:IsPurgable()
-- 	return true
-- end

-- function modifier_pathfinder_dk_dragon_tail:CheckState()
-- 	return {
-- 		[MODIFIER_STATE_STUNNED] = true,
-- 	}
-- end

-- function modifier_pathfinder_dk_dragon_tail:DeclareFunctions()
-- 	return {
-- 		MODIFIER_PROPERTY_OVERRIDE_ANIMATION,
-- 	}
-- end

-- function modifier_pathfinder_dk_dragon_tail:GetOverrideAnimation( params )
-- 	return ACT_DOTA_DISABLED
-- end

-- function modifier_pathfinder_dk_dragon_tail:GetEffectName()
-- 	return "particles/generic_gameplay/generic_stunned.vpcf"
-- end

-- function modifier_pathfinder_dk_dragon_tail:GetEffectAttachType()
...
modifier_pathfinder_dk_dragon_tail_passive	= class({
	IsHidden				= function(self) return true end,
	IsPurgable	  			= function(self) return false end,
	IsDebuff	  			= function(self) return false end,		
})

function modifier_pathfinder_dk_dragon_tail_passive:DeclareFunctions()
	local funcs =
	{
...
function modifier_pathfinder_dk_dragon_tail_passive:OnAttackLanded(params)
	if IsServer() and self:GetParent() == params.target and self:GetParent():HasAbility("pathfinder_dk_dragon_tail_passive") then
		local chance = self:GetParent():FindAbilityByName("pathfinder_dk_dragon_tail_passive"):GetLevelSpecialValueFor("chance",1)
...
modifier_pathfinder_dk_dragon_tail_bonus_shard_range	= class({
	IsHidden				= function(self) return true end,
	IsPurgable	  			= function(self) return false end,
	IsDebuff	  			= function(self) return false end,		
...
function modifier_pathfinder_dk_dragon_tail_bonus_shard_range:OnCreated()
	self:SetStackCount(0)
	self:StartIntervalThink(1.5)
end

function modifier_pathfinder_dk_dragon_tail_bonus_shard_range:OnIntervalThink()
	if IsServer() then
		local caster = self:GetCaster()
		
		if caster:HasAbility("pathfinder_dk_dragon_tail_bounce") then
			self:SetStackCount(caster:FindAbilityByName("pathfinder_dk_dragon_tail_bounce"):GetLevelSpecialValueFor("bonus_range",1))
		end				
	end
end
