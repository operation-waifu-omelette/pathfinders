...
function BehaviorOverwhelmingOdds:Evaluate()
	...
	if self.overwhelmingOddsAbility and self.overwhelmingOddsAbility:IsFullyCastable() then
		local enemies = FindUnitsInRadius( DOTA_TEAM_BADGUYS, thisEntity:GetOrigin(), nil, 12000, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NO_INVIS, 0, false )
		if ( #enemies > 0 )  then
			desire = 7			
		end
	end	
...
function BehaviorOverwhelmingOdds:Begin()
	...
	if self.overwhelmingOddsAbility and self.overwhelmingOddsAbility:IsFullyCastable() then
		local enemies = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetOrigin(), nil, 12000, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_FARTHEST, false )
		if #enemies == 0 then			
			return nil
		end

		for _,enemy in pairs(enemies) do
			if (enemy:GetAbsOrigin() - thisEntity:GetAbsOrigin()):Length2D() > 4000 then				
