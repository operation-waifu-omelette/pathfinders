...
LinkLuaModifier("modifier_ascension_plasma_field_display", "modifiers/modifier_ascension_plasma_field_display",
...
function CMapEncounter:constructor(hRoom, szEncounterName)
    ...
    if bSpawnExplosiveBarrels then
        if self.hRoom:GetType() == ROOM_TYPE_ENEMY then
            self.nExplosiveBarrelsToSpawn = self:RoomRandomInt(DEFAULT_MIN_BARRELS_ENEMY_ENC,
                DEFAULT_MAX_BARRELS_ENEMY_ENC)
        end
    end
...
function CMapEncounter:GetTreasureRandomStream()
    if self.hTreasureRandomStream == nil then
        self.hTreasureRandomStream = CreateUniformRandomStream(
            GameRules.Aghanim:GetRandomSeed() + MakeStringToken(self:GetRoom():GetName()))
    end
...
function CMapEncounter:SelectAscensionAbilities()
        ...
        if self:GetRoom():GetType() == ROOM_TYPE_BOSS then
            if GameRules.Aghanim.bIsInTournamentMode and #TRIALS_BOSS_ASCENSION_ABILITIES > 0 then
                for i = 1, #TRIALS_BOSS_ASCENSION_ABILITIES do
                    print("Encounter " .. self.szEncounterName .. " added trial ascension ability " ..
                              TRIALS_BOSS_ASCENSION_ABILITIES[i])
                ...
                if GameRules.Aghanim:GetAscensionLevel() > AGHANIM_ASCENSION_MAGICIAN and
                    PlayerResource:GetPlayerCountForTeam(DOTA_TEAM_GOODGUYS) > 2 then
                    local nIndex = self:RoomRandomInt(1, #APEX_BOSS_ASCENSION_ABILITIES)
                    print("Encounter " .. self.szEncounterName .. " added boss ascension ability " ..
                              APEX_BOSS_ASCENSION_ABILITIES[nIndex])
    ...
    if GameRules.Aghanim.bIsInTournamentMode and #TRIALS_ASCENSION_ABILITIES > 0 then
        ...
        for abilityName, hPossibleAbility in pairs(ASCENSION_ABILITIES) do
            ...
            if hPossibleAbility.nRestrictToAct ~= nil then
                ...
                if hPossibleAbility.szRequiredBoss ~= nil and hPossibleAbility.szRequiredBoss ~=
                    GameRules.Aghanim:GetBossUnitForAct(hPossibleAbility.nRestrictToAct) then
                    goto continue
                end
            end
    ...
    if nEliteLevel > 0 then
        ...
        for i = nStart, nDesiredEliteAbilityCount do
            ...
            print("Encounter " .. self.szEncounterName .. " added ELITE ascension ability " ..
                      vecEliteAbilityOptions[nPick])
...
function CMapEncounter:Start()
    ...
    for i = 0, GameRules:NumDroppedItems() - 1 do
        ...
        if hDroppedItem then
            ...
            if hContainedItem and hContainedItem:IsNeutralDrop() then
                ...
                local nFXIndex = ParticleManager:CreateParticle("particles/items2_fx/neutralitem_teleport.vpcf",
    ...
    local nEntityKilledGameEvent = ListenToGameEvent("entity_killed", Dynamic_Wrap(getclass(self), "OnEntityKilled"),
        self)
    ...
    if self.bCalculateRewardsFromUnitCount then
        ...
        if self.nMaxSpawnedUnitCount == 0 then
            print("*** WARNING : Encounter " .. self.szEncounterName ..
                      " indicates 0 units to be spawned.. Check your GetMaxSpawnedUnitCount()")
        end
    end
    ...
    if self.flEnrageTimer then
        CustomNetTables:SetTableValue("room_data", "enrage_timer", {
            active = true,
    ...
    CustomNetTables:SetTableValue("room_data", "status", {
        complete = false
    })
            ...
            if nChestsSpawned < self.nChestsToSpawn then
                ...
                if bSpawnChest then
                    ...
                    if hUnit ~= nil then
                        ...
                        printf("ERROR -- CMapEncounter:SpawnChests: Failed to spawn chest named \"%s\"",
                            chestData.szNPCName)
                    end
                end
            end
...
function CMapEncounter:SpawnBreakableContainers()
    ...
    for i, breakableData in ipairs(BreakablesData) do
        ...
        if (breakableData.szSpawnerName == nil) then
            printf(
                "CMapEncounter:SpawnBreakableContainers() - ERROR: No szSpawnerName specified for this breakable container.")
            return
        end
            ...
            for i, hSpawner in pairs(hSpawners) do
                if nCratesSpawned < self.nCratesToSpawn then
                    local vSpawnLoc = hSpawner:GetOrigin() +
                                          RandomVector(RandomFloat(0, breakableData.nMaxSpawnDistance))
                    ...
                    if bSpawnBreakable then
                        ...
                        local breakableTable = {
                            ...
                            origin = tostring(vSpawnLoc.x) .. " " .. tostring(vSpawnLoc.y) .. " " ..
                                tostring(vSpawnLoc.z),
...
function CMapEncounter:SpawnExplosiveBarrels()
    ...
    for i, barrelData in ipairs(ExplosiveBarrelData) do
        ...
        if (barrelData.szSpawnerName == nil) then
            printf(
                "CMapEncounter:SpawnExplosiveBarrels() - ERROR: No szSpawnerName specified for this explosive barrel.")
            return
        end
...
function CMapEncounter:SetPortalTriggerSpawner(szSpawnerName, flDelay)
    ...
    if self.Spawners[szSpawnerName] == nil then
        print("WARNING: CMapEncounter:SetPortalTriggerSpawner specified unknown spawner ( " .. szSpawnerName ..
                  ") in encounter " .. self.szEncounterName .. "! ")
        return
    end
    ...
    for _, hPortalSpawner in pairs(self.PortalSpawnersV2) do
        if hPortalSpawner.schedule ~= nil then
            if hPortalSpawner.schedule.bWaitingForTrigger == true then
                ...
                hPortalSpawner.schedule.flStartTime = GameRules:GetGameTime() + hPortalSpawner.schedule.flDelay +
                                                          self.flPortalTriggerDelay
            end
        end
    end
...
function CMapEncounter:SetSpawnerSchedule(szSpawnerName, waveScheduleInput)
    ...
    for i = 1, #waveScheduleInput do
        if nLastTime > waveScheduleInput[i].Time then
            print("ERROR: BeginSpawnerSchedule: Wave schedule ( " .. szSpawnerName ..
                      ") not specified in increasing order of time in encounter " .. self.szEncounterName)
            return
        end
    ...
    if hSpawner == nil then
        print("ERROR: BeginSpawnerSchedule: Spawner ( " .. szSpawnerName .. ") isn't found in this encounter " ..
                  self.szEncounterName)
        return
    end
...
function CMapEncounter:GenerateSpawnFocusPath(szSpawnerName, flSpeed, flDesiredRadiusInput, flMaxTime)
    ...
    if hSpawner == nil then
        print("ERROR: GenerateSpawnFocusPath: Spawner ( " .. szSpawnerName .. ") isn't found in encounter " ..
                  self.szEncounterName)
        return nil
    end
    ...
    if flMaxTime == nil or flMaxTime <= 0 then
        if hSpawner.schedule == nil then
            print("ERROR: GenerateSpawnFocusPath: Didn't specify duration for " .. szSpawnerName .. " in encounter " ..
                      self.szEncounterName)
            return nil
        end
    ...
    while flCurrDist < flMaxDist do
        ...
        if bIdenticalPoint == false then
            table.insert(spawnFocusPath.vecPathNodes, {
                Time = flTime,
                Position = vNewPos
            })
        end
...
function CMapEncounter:ComputeSpawnFocusPosition(spawnFocusPath, flTime)
    ...
    for i = 1, #spawnFocusPath.vecPathNodes do
        if flTime < spawnFocusPath.vecPathNodes[i].Time then
            ...
            local v = Vector(prev.Position.x + vDelta.x * t, prev.Position.y + vDelta.y * t,
                prev.Position.z + vDelta.z * t)
            return v
        end
    end
...
function CMapEncounter:AddAscensionAbilities(hEnemyCreature)
    ...
    if bIsGlobal == false then
        ...
        if self.hRoom:GetType() == ROOM_TYPE_BOSS then
            if (GameRules.Aghanim.bIsInTournamentMode == false or #TRIALS_BOSS_ASCENSION_ABILITIES == 0) and
                GameRules.Aghanim:GetAscensionLevel() < AGHANIM_ASCENSION_MAGICIAN then
...
function CMapEncounter:OnEnemyCreatureSpawned(hEnemyCreature)
    ...
    if not bIsDummy then
        if hEnemyCreature:GetUnitName() ~= "npc_dota_creature_bonus_chicken" and hEnemyCreature:GetUnitName() ~=
            "npc_dota_creature_bonus_greevil" then
...
function CMapEncounter:OnSpawnerFinished(hSpawner, hSpawnedUnits)
    ...
    local bIsPortalTriggerUnit = (self.szPortalTriggerSpawner ~= nil) and
                                     (self.szPortalTriggerSpawner == hSpawner:GetSpawnerName())
...
function CMapEncounter:GrantRewardsForKill(hVictim, hAttacker, nUnitCount)
    ...
    if hVictim.bSuppressRewardsOnDeath == nil or hVictim.bSuppressRewardsOnDeath == false then
        ...
        if self.bCalculateRewardsFromUnitCount and self.nMaxSpawnedUnitCount > 0 then
            ...
            if self.nRemainingXPFromEnemies > 0 then
                ...
                for _, Hero in pairs(Heroes) do
                    if Hero ~= nil and not Hero:HasModifier("modifier_hero_select") and Hero:IsRealHero() and
                        Hero:GetTeamNumber() == DOTA_TEAM_GOODGUYS then
        ...
        if self.nNumBPToDrop > 0 then
            local nPct = math.max(100 / nEstimatedUnitCount, 1)
            if RollPercentage(nPct) then
                self:DropCurrencyFromUnit(hVictim, hAttacker,
                    RandomInt(BATTLE_POINT_MIN_DROP_VALUE, BATTLE_POINT_MAX_DROP_VALUE), true, false)
    ...
    if hVictim:IsCreature() then
        ...
        if hAttacker and hAttacker:IsOwnedByAnyPlayer() then
            ...
            ParticleManager:ReleaseParticleIndex(ParticleManager:CreateParticleForPlayer(
                "particles/dark_moon/darkmoon_last_hit_effect.vpcf", PATTACH_ABSORIGIN_FOLLOW, hVictim,
...
function CMapEncounter:DropItemFromRoomRewardContainer(hContainer, szItemName, bAnnounce)
    for szNeutralItem, v in pairs(PRICED_ITEM_REWARD_LIST) do
        if szNeutralItem == szItemName then
            DropNeutralItemAtPositionForHero(szItemName, hContainer:GetAbsOrigin() + RandomVector(RandomFloat(50, 150)),
                PlayerResource:GetSelectedHeroEntity(0), -1, true)
            return
        end
    end
...
function CMapEncounter:SpawnEndLevelEntities()
    ...
    for _, hTemplateEnt in pairs(vExitTemplate:GetSpawnedEntities()) do
        ...
        if nDepth == 6 or nDepth == 11 or nDepth == 13 or nDepth == 17 then
            if hTemplateEnt:GetName() == "shop" or hTemplateEnt:GetName() == "shop_trigger" or hTemplateEnt:GetName() ==
                "shop_obstruction" or hTemplateEnt:GetName() == "shop_particles" or hTemplateEnt:GetName() ==
                "neutral_stash" then
    ...
    if self.hRoom:HasCrystal() then
        local hCrystal = CreateUnitByName("npc_dota_story_crystal", self.vRoomRewardCratePos + Vector(0, 350, 0), true,
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
        ...
        if hPlayerHero then
            if HEAL_ON_ENCOUNTER_COMPLETE then
                ...
                hPlayerHero:AddNewModifier(hPlayerHero, nil, "modifier_item_aeon_disk_buff", {
                    duration = 7
                })
            ...
            if hPlayerHero:GetUnitName() == "npc_dota_hero_dawnbreaker" then
                if hPlayerHero:HasAbility("dawnbreaker_solar_guardian_lua_permanent") then
                    if hPlayerHero:FindAbilityByName("dawnbreaker_solar_guardian_lua_permanent").thinker and
                        IsValidEntity(hPlayerHero:FindAbilityByName("dawnbreaker_solar_guardian_lua_permanent").thinker) then
            ...
            if hItem then
                if hItem:GetAbilityName() == "item_bottle" then
                    ...
                    hItem:SetCurrentCharges(math.min(hItem:GetCurrentCharges() + AGHANIM_ENCOUNTER_BOTTLE_CHARGES,
                        nMaxCharges))
            ...
            local nFXIndex = ParticleManager:CreateParticle("particles/econ/events/ti8/mekanism_ti8.vpcf",
                PATTACH_ABSORIGIN_FOLLOW, hPlayerHero)
            ...
            if hPlayerHero:GetUnitName() == "npc_dota_hero_sniper" then
                ...
                local hBigGameHunterAbility = hPlayerHero:FindAbilityByName(
                ...
                if hBigGameHunterAbility ~= nil and hBigGameHunterAbility:GetLevel() > 0 then
                    ...
                    if hLimiterBuff == nil then
                        hLimiterBuff = hPlayerHero:AddNewModifier(hPlayerHero, hBigGameHunterAbility,
...
function CMapEncounter:CreateRewardCrate()
    ...
    local hRewardCrate = CreateUnitByName("npc_treasure_chest", self.vRoomRewardCratePos, true, nil, nil,
        DOTA_TEAM_GOODGUYS)
    ...
    if self.hRoom:GetType() == ROOM_TYPE_ENEMY then
        ...
        if self.nNumBPToDrop > 0 then
            for i = 1, self.nNumBPToDrop do
                self:DropCurrencyFromUnit(hRewardCrate, hRewardCrate,
                    RandomInt(BATTLE_POINT_MIN_DROP_VALUE, BATTLE_POINT_MAX_DROP_VALUE), true, false)
            end
        end
    end
...
function CMapEncounter:GenerateRewards()
    ...
    local vecBPRewards = {}
    if nBPReward > 0 then
        vecBPRewards = GameRules.Aghanim:GrantAllPlayersPoints(nBPReward, true, "completing " .. self.szEncounterName ..
            " at depth " .. tostring(self.hRoom:GetDepth()))
    end

    local vecArcaneFragmentRewards = {}
    if nArcaneFragmentsReward > 0 then
        vecArcaneFragmentRewards = GameRules.Aghanim:GrantAllPlayersPoints(nArcaneFragmentsReward, false,
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
        local vecPlayerRewards = GetRoomRewards(self.hRoom:GetDepth(), self.hRoom:GetType(), bHardRoom, nPlayerID,
            vecAbilityNamesToExclude)
