...
function CMapEncounter_BonusBase:Start()
    ...
    self.nItemPickedUpListener = ListenToGameEvent("dota_item_picked_up",
        Dynamic_Wrap(getclass(self), "OnItemPickedUp"), self)
    self.nGoldReward = 0
end
...
function CMapEncounter_BonusBase:OnItemPickedUp(event)
    ...
    if event.HeroEntityIndex and event.HeroEntityIndex ~= 0 then
    ...
    if hCollector and hCollector:IsHero() and item and item:GetAbilityName() == "item_bag_of_gold" then
        ...
        self.PlayerGoldCollected[hCollector:GetPlayerID()]["bags"] =
            self.PlayerGoldCollected[hCollector:GetPlayerID()]["bags"] + 1
        self.PlayerGoldCollected[hCollector:GetPlayerID()]["gold"] =
            self.PlayerGoldCollected[hCollector:GetPlayerID()]["gold"] + item:GetCurrentCharges()
