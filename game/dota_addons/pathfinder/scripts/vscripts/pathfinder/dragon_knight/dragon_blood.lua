...
LinkLuaModifier( "modifier_pathfinder_dk_dragon_blood", "pathfinder/dragon_knight/dragon_blood", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_dk_dragon_blood_damage", "pathfinder/dragon_knight/dragon_blood", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_dk_dragon_blood_active", "pathfinder/dragon_knight/dragon_blood", LUA_MODIFIER_MOTION_NONE )

function pathfinder_dk_dragon_blood:GetIntrinsicModifierName()
	return "modifier_pathfinder_dk_dragon_blood"
end

function pathfinder_dk_dragon_blood:GetCastRange(vLocation, hTarget)
	if self:GetCaster():HasModifier("modifier_pathfinder_dk_dragon_blood_active") then
		return self:GetCaster():GetModifierStackCount("modifier_pathfinder_dk_dragon_blood_active", self:GetCaster())
	end
end

function pathfinder_dk_dragon_blood:GetBehavior()
	if self:GetCaster():HasModifier("modifier_pathfinder_dk_dragon_blood_active") then
...
function pathfinder_dk_dragon_blood:OnAbilityPhaseStart()
	if not IsServer() or not self:GetCaster():HasModifier("modifier_pathfinder_dk_dragon_blood_active") then return end

	if self:GetCaster():HasModifier("modifier_pathfinder_dk_elder_dragon_form") then
...
function pathfinder_dk_dragon_blood:OnSpellStart()
	if not IsServer() or not self:GetCaster():HasModifier("modifier_pathfinder_dk_dragon_blood_active") then return end

	local shard = self:GetCaster():FindAbilityByName("pathfinder_dk_dragon_blood_active")
...
modifier_pathfinder_dk_dragon_blood = {}

function modifier_pathfinder_dk_dragon_blood:IsHidden()
	return true
end

function modifier_pathfinder_dk_dragon_blood:OnCreated()
...
function modifier_pathfinder_dk_dragon_blood:OnIntervalThink()
	if not self:GetCaster():HasModifier("modifier_pathfinder_dk_dragon_blood_damage") and self:GetCaster():HasAbility("pathfinder_dk_dragon_blood_damage") then
		self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_pathfinder_dk_dragon_blood_damage", {})
	end

	if self:GetCaster():HasAbility("pathfinder_dk_dragon_blood_gold") then
		local shard = self:GetCaster():FindAbilityByName("pathfinder_dk_dragon_blood_gold")
	...
	if self:GetParent():HasAbility("pathfinder_dk_dragon_blood_active") then
		self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_pathfinder_dk_dragon_blood_active", {})
	end
end

function modifier_pathfinder_dk_dragon_blood:DeclareFunctions()
	return {
...
function modifier_pathfinder_dk_dragon_blood:GetModifierConstantHealthRegen()
	if not self:GetParent():PassivesDisabled() then
		return self:GetAbility():GetSpecialValueFor( "bonus_health_regen" ) + self.bonus_regen
	end
end

function modifier_pathfinder_dk_dragon_blood:GetModifierPhysicalArmorBonus()
	if not self:GetParent():PassivesDisabled() then		
		return self:GetAbility():GetSpecialValueFor( "bonus_armor" ) + self.bonus_armor
	end
end

function modifier_pathfinder_dk_dragon_blood:GetModifierMagicalResistanceBonus()
	if not self:GetParent():PassivesDisabled() then
		return self.magic_resist
	end
end

function modifier_pathfinder_dk_dragon_blood:AddCustomTransmitterData( )
	return
	{
...
function modifier_pathfinder_dk_dragon_blood:HandleCustomTransmitterData( data )
...
modifier_pathfinder_dk_dragon_blood_damage	= class({
...
function modifier_pathfinder_dk_dragon_blood_damage:OnCreated()
	self:SetStackCount(0)

	if not IsServer() then return end

	self.max_stack_mult = self:GetParent():FindAbilityByName("pathfinder_dk_dragon_blood_damage"):GetLevelSpecialValueFor("max_stack",1)
	self:StartIntervalThink(1)
end

function modifier_pathfinder_dk_dragon_blood_damage:OnIntervalThink()
	if not IsServer() then return end
...
function modifier_pathfinder_dk_dragon_blood_damage:DeclareFunctions()
	local funcs =
	{
...
function modifier_pathfinder_dk_dragon_blood_damage:OnAttackLanded(params)
...
modifier_pathfinder_dk_dragon_blood_active	= class({
...
function modifier_pathfinder_dk_dragon_blood_active:OnCreated()
	if not IsServer() then return end
	local radius = self:GetParent():FindAbilityByName("pathfinder_dk_dragon_blood_active"):GetLevelSpecialValueFor("radius",1)
