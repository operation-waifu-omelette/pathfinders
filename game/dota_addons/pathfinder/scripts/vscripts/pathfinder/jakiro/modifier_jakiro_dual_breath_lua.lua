...
LinkLuaModifier( "modifier_jakiro_ice_path_shard_passive", "pathfinder/jakiro/modifier_jakiro_ice_path_lua_thinker", LUA_MODIFIER_MOTION_NONE )
--------------------------------------------------------------------------------
-- Classifications
...
function modifier_jakiro_dual_breath_lua:OnCreated( kv )
	-- references
	local distance = self:GetAbility():GetLevelSpecialValueFor( "range", self:GetAbility():GetLevel() - 1 ) + FindTalentValue(self:GetCaster(), "special_bonus_unique_jakiro_dual_breath_range")
