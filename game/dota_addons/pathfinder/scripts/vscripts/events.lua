...
function CAghanim:OnGameRulesStateChange()
    ...
    if nNewState == DOTA_GAMERULES_STATE_HERO_SELECTION then
        ...
        if IsServer() then
            local sec = GameRules:GetStateTransitionTime()
            Timers:CreateTimer(function()
                for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
                    if PlayerResource:IsValidPlayerID(nPlayerID) then
                        local pName = PlayerResource:GetPlayerName(nPlayerID)
...
function CAghanim:AddResultToSignOut()
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
        if PlayerResource:IsValidPlayerID(nPlayerID) then
            ...
            self.SignOutTable["player_list"][nPlayerID]["current_ascension_level"] =
                PlayerResource:GetEventGameCustomActionClaimCountByName(nPlayerID,
                    "ti10_event_game_current_ascension_level")
...
function CAghanim:LockFaceVoiceLine(heroName)
    ...
    elseif hero == "winter_wyvern" then
        hero = "auroth"
    elseif hero == "nevermore" then
        hero = "shadowfiend"
    elseif hero == "tidehunter" then
        hero = "tide"
    elseif hero == "dragon_knight" then
        hero = "dk"
    end
    ...
    elseif hero == "hoodwink" then
        EmitGlobalSound("hoodwink_hoodwink_idle_01")
    elseif hero == "dawnbreaker" then
        EmitGlobalSound("dawnbreaker_valora_intro_07_03")
    else
        EmitGlobalSound("announcer_dlc_bastion_announcer_pick_" .. hero .. "_follow")
    end
end

function CAghanim:OnPickFace(keys)
    if not IsServer() then
        return
    end

    local hero = keys.hero
    local player = PlayerResource:GetPlayer(keys.id)
    player:SetSelectedHero(hero)
    Timers(0.5, function()
        CAghanim:LockFaceVoiceLine(string.sub(hero, 15))
    end)
end
...
function CAghanim:OrderFilter(keys)    
    return true
end
...
function CAghanim:OnNPCSpawned_PlayerHero(event)
    ...
    if self.bDataSent == nil then
        ...
        CustomNetTables:SetTableValue("special_ability_upgrades", tostring(0),
            SPECIAL_ABILITY_UPGRADES[hPlayerHero:GetUnitName()])
        self.bDataSent = true
    end
    -- end)
end
