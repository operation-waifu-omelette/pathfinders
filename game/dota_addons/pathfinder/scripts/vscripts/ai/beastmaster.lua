...
function BehaviorPrimalRoar:Evaluate()
	...
	self.primalRoarAbility = thisEntity:FindAbilityByName( "primal_roar_long" )
	if self.primalRoarAbility and self.primalRoarAbility:IsFullyCastable() then
		local enemies = FindUnitsInRadius( DOTA_TEAM_BADGUYS, thisEntity:GetOrigin(), nil, 1250, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NO_INVIS, 0, false )
