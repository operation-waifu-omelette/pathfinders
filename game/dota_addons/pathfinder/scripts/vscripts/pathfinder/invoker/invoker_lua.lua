...
invoker_lua = invoker_lua or class({})
...
invoker_lua_quas = class({})

LinkLuaModifier("modifier_invoker_lua_quas", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)

function invoker_lua_quas:ProcsMagicStick() return false end

function invoker_lua_quas:OnUpgrade()
	if not IsServer() then return end

	CustomNetTables:SetTableValue("player_table", "quas_level"..tostring(self:GetCaster():GetPlayerOwnerID()), {self:GetLevel()})
end

function invoker_lua_quas:OnSpellStart()
	if IsServer() then
		...
		invoker_lua:update_orbs(caster, self, "invoker_lua_quas", "particles/units/heroes/hero_invoker/invoker_quas_orb.vpcf")
	end
end
...
modifier_invoker_lua_quas = modifier_invoker_lua_quas or class({})

function modifier_invoker_lua_quas:IsBuff() 		return true  end
function modifier_invoker_lua_quas:IsHidden() 		return false end
function modifier_invoker_lua_quas:IsDebuff() 		return false end
function modifier_invoker_lua_quas:IsPurgable() 	return false end
function modifier_invoker_lua_quas:RemoveOnDeath()  return false end
function modifier_invoker_lua_quas:DeclareFunctions()
	local funcs = {
...
function modifier_invoker_lua_quas:OnCreated(kv)
	if IsServer() then 
...
function modifier_invoker_lua_quas:GetModifierBonusStats_Strength()
	return self.bonus_strength
end

function modifier_invoker_lua_quas:GetModifierConstantHealthRegen() 
	return self.health_regen_per_instance
end

function modifier_invoker_lua_quas:GetAttributes()
...
--	Invoker: WEX
---------------------------------------------------------------------------------------------------------------------
invoker_lua_wex = class({})
LinkLuaModifier("modifier_invoker_lua_wex", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)

function invoker_lua_wex:ProcsMagicStick() return false end

function invoker_lua_wex:OnUpgrade()
	if not IsServer() then return end

	CustomNetTables:SetTableValue("player_table", "wex_level"..tostring(self:GetCaster():GetPlayerOwnerID()), {self:GetLevel()})
end

function invoker_lua_wex:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		...
		invoker_lua:update_orbs(caster, self, "invoker_lua_wex", "particles/units/heroes/hero_invoker/invoker_wex_orb.vpcf")
	end
end
...
---------------------------------------------------------------------------------------------------------------------
modifier_invoker_lua_wex = modifier_invoker_lua_wex or class({})
function modifier_invoker_lua_wex:IsBuff() 		return true  end
function modifier_invoker_lua_wex:IsHidden() 		return false end
function modifier_invoker_lua_wex:IsDebuff() 		return false end
function modifier_invoker_lua_wex:IsPurgable() 	return false end
function modifier_invoker_lua_wex:RemoveOnDeath()	return false end
function modifier_invoker_lua_wex:DeclareFunctions()
	local funcs = {
...
function modifier_invoker_lua_wex:OnCreated(kv)
	if IsServer() then 
...
function modifier_invoker_lua_wex:GetModifierBonusStats_Agility()
	return self.bonus_agility
end

-- Movement speed buff is working but dosent show up in hero stats
function modifier_invoker_lua_wex:GetModifierMoveSpeedBonus_Percentage() 
	return self.move_speed_per_instance
end

function modifier_invoker_lua_wex:GetModifierAttackSpeedBonus_Constant() 
	return self.attack_speed_per_instance
end

function modifier_invoker_lua_wex:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
invoker_lua_exort = class({})

LinkLuaModifier("modifier_invoker_lua_exort", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)

function invoker_lua_exort:ProcsMagicStick() return false end

function invoker_lua_exort:OnUpgrade()
	if not IsServer() then return end

	CustomNetTables:SetTableValue("player_table", "exort_level"..tostring(self:GetCaster():GetPlayerOwnerID()), {self:GetLevel()})
end

function invoker_lua_exort:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		...
		invoker_lua:update_orbs(caster, self, "invoker_lua_exort", "particles/units/heroes/hero_invoker/invoker_exort_orb.vpcf")
	end
end
...
---------------------------------------------------------------------------------------------------------------------
modifier_invoker_lua_exort = modifier_invoker_lua_exort or class({})
function modifier_invoker_lua_exort:IsBuff() 			return true  end
function modifier_invoker_lua_exort:IsHidden() 		return false end
function modifier_invoker_lua_exort:IsDebuff() 		return false end
function modifier_invoker_lua_exort:IsPurgable() 		return false end
function modifier_invoker_lua_exort:RemoveOnDeath()	return false end
function modifier_invoker_lua_exort:DeclareFunctions()
	local funcs = {
...
function modifier_invoker_lua_exort:OnCreated(kv)
	if IsServer() then 
...
function modifier_invoker_lua_exort:GetModifierBonusStats_Intellect()
	return self.bonus_intelligence
end

function modifier_invoker_lua_exort:GetModifierBaseAttack_BonusDamage() 
	return self.bonus_damage_per_instance
end

function modifier_invoker_lua_exort:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
---------------------------------------------------------------------------------------------------------------------
function invoker_lua:update_orbs(caster, ability, invoked_orb, particle_path)
	if IsServer() then 
		-- need to make sure we dont mess up orb order
		invoker_lua.orb_sync = false
		...
		invoker_lua:update_orb_modifiers(caster, ability, remove_orb)

		-- Change base_attack_effect depending on current orb setup
		invoker_lua:update_base_attack(caster) 
	end
end
...
---------------------------------------------------------------------------------------------------------------------
function invoker_lua:update_orb_modifiers(caster, ability, orb_to_remove)	
	-- Find modifier to remove...
	local orb_modifier = nil
	if orb_to_remove ~= nil then
		if orb_to_remove:GetName() == "invoker_lua_quas" then
			orb_modifier = "modifier_invoker_lua_quas"
		elseif orb_to_remove:GetName() == "invoker_lua_wex" then
			orb_modifier = "modifier_invoker_lua_wex"
		elseif orb_to_remove:GetName() == "invoker_lua_exort" then
			orb_modifier = "modifier_invoker_lua_exort"
		end
	end
	...
	if new_orb == "invoker_lua_quas" then
		...
		caster:AddNewModifier(caster, ability, "modifier_invoker_lua_quas", { 	duration = -1, 
	...
	elseif new_orb == "invoker_lua_wex" then
		...
		caster:AddNewModifier(caster, ability, "modifier_invoker_lua_wex", {	duration = -1, 
	...
	elseif new_orb == "invoker_lua_exort" then
		...
		caster:AddNewModifier(caster, ability, "modifier_invoker_lua_exort", {	duration = -1, 
...
function invoker_lua:update_base_attack(caster) 
	local quas 	= 0 
	local wex 	= 0 
	local exort = 0 
	...
	for _,orb in pairs(caster.invoked_orbs) do 
		...
		if orb_type == 'invoker_lua_quas' then 
			quas = quas + 1
		elseif orb_type == 'invoker_lua_wex' then
			wex = wex + 1
		elseif orb_type == 'invoker_lua_exort' then
			exort = exort + 1
		end
	end
...
---------------------------------------------------------------------------------------------------------------------
modifier_invoker_lua_aghanim_buff = class({})

function modifier_invoker_lua_aghanim_buff:IsPurgable()	return false end
function modifier_invoker_lua_aghanim_buff:IsHidden() 		return true  end
function modifier_invoker_lua_aghanim_buff:IsDebuff() 		return false end
function modifier_invoker_lua_aghanim_buff:RemoveOnDeath() return false end

function modifier_invoker_lua_aghanim_buff:OnCreated()
	if IsServer() then
		local caster 	= self:GetCaster()
		local quas 		= caster:FindAbilityByName("invoker_lua_quas")
		local wex 		= caster:FindAbilityByName("invoker_lua_wex")
		local exort 	= caster:FindAbilityByName("invoker_lua_exort")
...
function modifier_invoker_lua_aghanim_buff:OnDestroy()
	if not IsServer() then return end
...
--	Invoker's Invoke
-------------------------------------------------------------------------------------------------------------------------
	invoker_lua_invoke = invoker_lua_invoke or class({})

	LinkLuaModifier("modifier_invoker_lua_invoke_buff", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
	LinkLuaModifier("modifier_invoker_lua_aghanim_buff", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)

	function invoker_lua_invoke:IsInnateAbility() return true end

	function invoker_lua_invoke:OnInventoryContentsChanged() 
		if IsServer() then
			if self:GetCaster():HasScepter() and (not self:GetCaster():HasModifier("modifier_invoker_lua_aghanim_buff")) then 
				self:GetCaster():AddNewModifier(self:GetCaster(), {}, "modifier_invoker_lua_aghanim_buff", {duration = -1})
			elseif not self:GetCaster():HasScepter() and self:GetCaster():HasModifier("modifier_invoker_lua_aghanim_buff") then
				self:GetCaster():RemoveModifierByName("modifier_invoker_lua_aghanim_buff")
			end
		end
	end

	function invoker_lua_invoke:GetCooldown(iLevel)
	...
	function invoker_lua_invoke:OnSpellStart()
		if IsServer() then
			local caster 				= self:GetCaster()
			local ability 				= self
			...
			local quas_orbs 		= caster:FindAllModifiersByName("modifier_invoker_lua_quas")
			local wex_orbs 			= caster:FindAllModifiersByName("modifier_invoker_lua_wex")
			local exort_orbs 		= caster:FindAllModifiersByName("modifier_invoker_lua_exort")
			...
			if num_quas_orbs + num_wex_orbs + num_exort_orbs == 3 then
				...
				if num_quas_orbs == 3 then
					spell_to_be_invoked = "invoker_lua_cold_snap"
				elseif num_quas_orbs == 2 and num_wex_orbs == 1 then
					spell_to_be_invoked = "invoker_lua_ghost_walk"
				elseif num_quas_orbs == 2 and num_exort_orbs == 1 then
					spell_to_be_invoked = "invoker_lua_ice_wall"
				elseif num_wex_orbs == 3 then
					spell_to_be_invoked = "invoker_lua_emp"
				elseif num_wex_orbs == 2 and num_quas_orbs == 1 then
					spell_to_be_invoked = "invoker_lua_tornado"
				elseif num_wex_orbs == 2 and num_exort_orbs == 1 then
					spell_to_be_invoked = "invoker_lua_alacrity"
				elseif num_exort_orbs == 3 then
					spell_to_be_invoked = "invoker_lua_sun_strike"
				elseif num_exort_orbs == 2 and num_quas_orbs == 1 then
					spell_to_be_invoked = "invoker_lua_forge_spirit"
				elseif num_exort_orbs == 2 and num_wex_orbs == 1 then
					spell_to_be_invoked = "invoker_lua_chaos_meteor"
				elseif num_quas_orbs == 1 and num_wex_orbs == 1 and num_exort_orbs == 1 then
					spell_to_be_invoked = "invoker_lua_deafening_blast"
				end
	...
	---------------------------------------------------------------------------------------------------------------------
	function invoker_lua_invoke:OnUpgrade()
		if IsServer() then
			if self.caster == nil then
				self.caster = self:GetCaster()
			end
			if self.ability == nil then
				self.ability = self.caster:FindAbilityByName("invoker_lua_invoke")
			end

			-- Remove old instance and create new...
			if self.caster:HasModifier("modifier_invoker_lua_invoke_buff") then
				self.caster:RemoveModifierByName("modifier_invoker_lua_invoke_buff")
			end

			self.caster:AddNewModifier(self.caster, self.ability, "modifier_invoker_lua_invoke_buff", {duration = -1})
		end
	end
	...
	---------------------------------------------------------------------------------------------------------------------
	LinkLuaModifier("modifier_invoker_lua_invoke_buff", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
	function invoker_lua_invoke:Invoke(caster)
		if IsServer() then 
	...
	---------------------------------------------------------------------------------------------------------------------
	modifier_invoker_lua_invoke_buff = class({})
	function modifier_invoker_lua_invoke_buff:IsPurgable()		return false end
	function modifier_invoker_lua_invoke_buff:IsHidden() 		return true  end
	function modifier_invoker_lua_invoke_buff:RemoveOnDeath() 	return false end
	function modifier_invoker_lua_invoke_buff:OnCreated()
		if IsServer() then
			self.caster 	= self:GetCaster()
	...
	function modifier_invoker_lua_invoke_buff:DeclareFunctions()
		local funcs = {
	...
	function modifier_invoker_lua_invoke_buff:GetModifierSpellAmplify_Percentage()
		return self.spell_amp
	end

	function modifier_invoker_lua_invoke_buff:GetModifierMagicalResistanceBonus()
		return self.magic_resist
	end
	function modifier_invoker_lua_invoke_buff:GetModifierBonusStats_Intellect()
		return self.int_buff
	end

	function modifier_invoker_lua_invoke_buff:GetCustomCooldownReductionStacking()
		return self.cooldown_reduction
	end

	function modifier_invoker_lua_invoke_buff:GetModifierSpellLifesteal()
		return self.spell_lifesteal
	end

	function modifier_invoker_lua_invoke_buff:OnAbilityExecuted(kv)
		if IsServer() then 
			local caster = self:GetParent()
			if kv.unit == caster then
				if kv.ability:GetName() == "invoker_invoke" then
					invoker_lua_invoke:Invoke(caster)
				end
			end
		end
	end
...
-------------------------------------------------------------------------------------------------------------------------
	...
	---------------------------------------------------------------------------------------------------------------------
		...
		LinkLuaModifier("modifier_invoker_lua_sun_strike_beam_only", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_sun_strike_incinerate", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		
		LinkLuaModifier("modifier_invoker_lua_sun_strike_thinker", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		
		modifier_invoker_lua_sun_strike_thinker	= modifier_invoker_lua_sun_strike_thinker or class({})
		
		function invoker_lua_sun_strike:GetCastAnimation()
			return ACT_DOTA_CAST_SUN_STRIKE
		end

		function invoker_lua_sun_strike:GetBehavior()
			if IsServer() then
		...
		-- function invoker_lua_sun_strike:CastFilterResultTarget( target )
			-- if target ~= nil and target == self:GetCaster() then
				-- return UF_SUCCESS
			-- end
		-- end

		function invoker_lua_sun_strike:OnSpellStart()
			...
			-- local target 				= self:GetCursorTarget()
			-- local ability 				= caster:FindAbilityByName("invoker_lua_sun_strike")
			...
			-- if double_tap then
				-- if caster:HasTalent("imba_special_bonus_unique_invoker_5") then 
					...
					-- invoker_lua_sun_strike:CastCataclysmSunStrike({
						-- caster 					= caster,
			...
			-- else
				...
				-- invoker_lua_sun_strike:CastSunStrike(
				-- {
					-- caster 				= caster,
					-- caster_team 		= caster_team,
					-- ability 			= ability,
			...
			if not self:GetCaster():HasTalent("invoker_lua_exort") then return end
			...
			CreateModifierThinker(self:GetCaster(), self, "modifier_invoker_lua_sun_strike_thinker", {
				...
				damage				= self:GetLevelSpecialValueFor("damage", self:GetCaster():FindAbilityByName("invoker_lua_exort"):GetLevel() - 1),
				bCataclysm			= bCataclysm
			}, self:GetCursorPosition(), self:GetCaster():GetTeamNumber(), false)
		end

		function invoker_lua_sun_strike:CastSunStrike(kv)
					...
					if hero:GetTeam() == caster_team then
						...
						if hero:GetPlayerID() == caster:GetPlayerID() then 
							CreateModifierThinker(
								caster, 
								ability, 
								"modifier_invoker_lua_sun_strike", 	
								{
									duration 			= delay,
									area_of_effect 		= area_of_effect,
						...
						-- Only show particle effects
							...
							CreateModifierThinker(
								caster, 
								ability, 
								"modifier_invoker_lua_sun_strike_beam_only", 	
								{
									duration 			= delay,
									area_of_effect 		= area_of_effect,
									target 				= hero:entindex(),
		...
		function invoker_lua_sun_strike:CastCataclysmSunStrike(kv)
			local caster 				= kv.caster
			local caster_team		 	= kv.caster_team
			local target_point 			= kv.target_point
			local ability 				= kv.ability
						...
						for _,hero2 in pairs(all_heroes) do
							...
							if hero2:GetPlayerID() == caster:GetPlayerID() then
								CreateModifierThinker(
									caster, 
									ability, 
									"modifier_invoker_lua_sun_strike_cataclysm", 	
									{
										duration 			= delay,
										area_of_effect 		= area_of_effect,
		...
		function invoker_lua_sun_strike:OnHit(caster, ability, enemies_hit, damage) 
		...
		function invoker_lua_sun_strike:GetAOERadius()
			return self:GetSpecialValueFor("area_of_effect")
		end 

		modifier_invoker_lua_sun_strike = class({})
		function modifier_invoker_lua_sun_strike:OnCreated(kv)
			if IsServer() then
				self.caster 			= self:GetCaster()
				...
				local invoker_exort 		= self.caster:FindAbilityByName("invoker_lua_exort")
				...
				local sun_strike_beam = ParticleManager:CreateParticleForPlayer(invoker_lua_sun_strike.ability_team_particle_effect, PATTACH_POINT, self.target, PlayerResource:GetPlayer(self.target:GetPlayerID()))
		...
		function modifier_invoker_lua_sun_strike:OnIntervalThink()
			if IsServer() then
--				EmitSoundOnLocationWithCaster(self.target_point, "Hero_Invoker.SunStrike.Charge", self.caster)
				...
				local sun_strike_beam = ParticleManager:CreateParticleForPlayer(invoker_lua_sun_strike.ability_outer_beam_effect, PATTACH_POINT, self.target, PlayerResource:GetPlayer(self.target:GetPlayerID()))
				...
				local sun_strike_beam2 = ParticleManager:CreateParticleForPlayer(invoker_lua_sun_strike.ability_outer_beam_effect, PATTACH_POINT, self.target, PlayerResource:GetPlayer(self.target:GetPlayerID()))
		...
		function modifier_invoker_lua_sun_strike:OnRemoved()
			if IsServer() then
				...
				if nearby_enemy_units ~= nil then
					for _,hero in pairs(nearby_enemy_units) do
						hero:AddNewModifier(self.caster, self.ability, "modifier_invoker_lua_sun_strike_incinerate", {duration = self.incinerate_duration, damage = self.damage})
				...
				if enemies_hit > 1 then 
					invoker_lua_sun_strike:OnHit(self.caster, self.ability, hit_table, self.damage)
				end
				...
				local sun_strike_crater = ParticleManager:CreateParticleForPlayer(invoker_lua_sun_strike.ability_particle_effect, PATTACH_POINT, self.target, PlayerResource:GetPlayer(self.target:GetPlayerID()))
		...
		modifier_invoker_lua_sun_strike_cataclysm = class({})
		function modifier_invoker_lua_sun_strike_cataclysm:OnCreated(kv)
			if IsServer() then
				self.caster 				= self:GetCaster()
				self.target_point 			= self:GetParent():GetAbsOrigin()
				self.target 				= EntIndexToHScript(kv.target)
		...
		function modifier_invoker_lua_sun_strike_cataclysm:OnRemoved()
			if IsServer() then
				--ParticleManager:DestroyParticle(sun_strike_beam3, false)
				local sun_strike_crater = ParticleManager:CreateParticleForPlayer(invoker_lua_sun_strike.ability_team_particle_effect, PATTACH_POINT, self.target, PlayerResource:GetPlayer(self.target:GetPlayerID()))
				ParticleManager:SetParticleControl(sun_strike_crater, 0, self.target_point)
				ParticleManager:SetParticleControl(sun_strike_crater, 1, Vector(self.area_of_effect,0, 0))
				...
				if nearby_enemy_units ~= nil then
					...
					invoker_lua_sun_strike:OnHit(self.caster, self.ability, nearby_enemy_units, self.damage)
				end
			end
		end

		modifier_invoker_lua_sun_strike_incinerate = class({})
		function modifier_invoker_lua_sun_strike_incinerate:IsDebuff() 		return true end
		function modifier_invoker_lua_sun_strike_incinerate:GetEffectName() 	return "particles/units/heroes/hero_phoenix/phoenix_fire_spirit_burn.vpcf" end
		function modifier_invoker_lua_sun_strike_incinerate:OnCreated(kv)
			if IsServer() then 
		...
		function modifier_invoker_lua_sun_strike_incinerate:OnIntervalThink()	
			if IsServer() then
				-- Apply damage
				local damage_table = {}
		...
		function modifier_invoker_lua_sun_strike_incinerate:OnRefresh()
			if IsServer() then 
				self:SetStackCount(self:GetStackCount() + 1)
			end
		end

		modifier_invoker_lua_sun_strike_beam_only = class({})
		function modifier_invoker_lua_sun_strike_beam_only:OnCreated(kv)
			if IsServer() then
				self.caster 			= self:GetCaster()
				self.target_point 		= self:GetParent():GetAbsOrigin()
		...
		function modifier_invoker_lua_sun_strike_beam_only:OnRemoved()
			if IsServer() then
		...
		----------------------------------------------
		-- MODIFIER_invoker_lua_SUN_STRIKE_THINKER --
		----------------------------------------------

		function modifier_invoker_lua_sun_strike_thinker:OnCreated(kv)
			if not IsServer() then return end

			self.area_of_effect 		= self:GetAbility():GetSpecialValueFor("area_of_effect")
			self.damage					= self:GetAbility():GetLevelSpecialValueFor("damage", self:GetCaster():FindAbilityByName("invoker_lua_exort"):GetLevel())
		...
		function modifier_invoker_lua_sun_strike_thinker:OnDestroy()
			if not IsServer() then return end
			...
			for location = 1, #self.sun_strike_points do
				...
				if #nearby_enemy_units > 0 then
					...
					for _, enemy in pairs(nearby_enemy_units) do					
						...
						incinerate_modifier = enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_invoker_lua_sun_strike_incinerate", {duration = self.incinerate_duration, damage = self.damage})
	...
	--	Invoker's Cold Snap
	---------------------------------------------------------------------------------------------------------------------
		invoker_lua_cold_snap = class({})
		LinkLuaModifier("modifier_invoker_lua_cold_snap", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_cold_snap_stun_duration", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_cold_snap_cooldown", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_cold_snap_buff", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		function invoker_lua_cold_snap:GetCastAnimation()
			return ACT_DOTA_CAST_COLD_SNAP
		end

		function invoker_lua_cold_snap:OnSpellStart()
			if IsServer() then 
				local caster 				= self:GetCaster()
				local target 				= self:GetCursorTarget()
				local ability 				= caster:FindAbilityByName("invoker_lua_cold_snap") 
				local quas_level 			= caster:FindAbilityByName("invoker_lua_quas"):GetLevel() - 1
				...
				if target:GetTeam() ~= caster:GetTeam() then
					...
					target:AddNewModifier(caster, ability, "modifier_invoker_lua_cold_snap", {duration = cold_snap_duration})
					target:AddNewModifier(caster, ability, "modifier_invoker_lua_cold_snap_stun_duration", {duration = freeze_duration * (1 - target:GetStatusResistance())}) 
					target:AddNewModifier(caster, ability, "modifier_invoker_lua_cold_snap_cooldown", {duration = freeze_cooldown * (1 - target:GetStatusResistance())})
					...
					target:AddNewModifier(caster, ability, "modifier_invoker_lua_cold_snap_buff", {duration = cold_snap_duration})
				end
			end
		end
		...
		---------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_cold_snap = class({})
		LinkLuaModifier("modifier_invoker_lua_cold_snap_stun_duration", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_cold_snap_cooldown", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		function modifier_invoker_lua_cold_snap:IsBuff() 			return false end
		function modifier_invoker_lua_cold_snap:IsHidden() 		return false end
		function modifier_invoker_lua_cold_snap:IsDebuff() 		return true  end
		function modifier_invoker_lua_cold_snap:IsPurgable() 		return true end
		function modifier_invoker_lua_cold_snap:IgnoreTenacity()	return true end
		function modifier_invoker_lua_cold_snap:GetEffectName() 	return "particles/units/heroes/hero_invoker/invoker_cold_snap_status.vpcf" end
		function modifier_invoker_lua_cold_snap:DeclareFunctions()
			local funcs = {
		...
		function modifier_invoker_lua_cold_snap:OnTakeDamage(kv) 
			if IsServer() then
				local caster 			= self:GetCaster()
				...
				if caster ~= target and target:HasModifier("modifier_invoker_lua_cold_snap") then
					local freeze_duration 			= ability:GetLevelSpecialValueFor("freeze_duration", quas_level) 
					local freeze_cooldown 			= ability:GetLevelSpecialValueFor("freeze_cooldown", quas_level) 
					local freeze_damage 			= ability:GetLevelSpecialValueFor("freeze_damage", quas_level)
					local damage_trigger 			= ability:GetSpecialValueFor("damage_trigger")
					...
					if kv.damage >= damage_trigger and not target:HasModifier("modifier_invoker_lua_cold_snap_cooldown") and target:IsAlive() then
						target:AddNewModifier(caster, ability, "modifier_invoker_lua_cold_snap_stun_duration", {duration = freeze_duration * (1 - target:GetStatusResistance())}) 
						target:AddNewModifier(caster, ability, "modifier_invoker_lua_cold_snap_cooldown", {duration = freeze_cooldown * (1 - target:GetStatusResistance())})
		...
		---------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_cold_snap_cooldown = class({})
		function modifier_invoker_lua_cold_snap_cooldown:IsHidden() 	return true  end
		function modifier_invoker_lua_cold_snap_cooldown:IsPurgable() 	return false end

		---------------------------------------------------------------------------------------------------------------------
		--	Cold Snap stun modifier
		---------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_cold_snap_stun_duration = class({})
		function modifier_invoker_lua_cold_snap_stun_duration:IsDebuff() 		return true  end
		function modifier_invoker_lua_cold_snap_stun_duration:IsPurgable() 	return false end
		function modifier_invoker_lua_cold_snap_stun_duration:IsHidden() 		return true  end
		function modifier_invoker_lua_cold_snap_stun_duration:GetEffectName() 	return "particles/units/heroes/hero_invoker/invoker_cold_snap.vpcf" end
		function modifier_invoker_lua_cold_snap_stun_duration:OnCreated(kv) 
			if IsServer() then
		...
		function modifier_invoker_lua_cold_snap_stun_duration:CheckState()
			local state = {
		...
		function modifier_invoker_lua_cold_snap_stun_duration:OnRemoved()
			if IsServer() then
		...
		---------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_cold_snap_buff = class({})
		function modifier_invoker_lua_cold_snap_buff:IsDebuff() 		return false end
		function modifier_invoker_lua_cold_snap_buff:IsHidden() 		return false end
		function modifier_invoker_lua_cold_snap_buff:GetEffectName() 	return "particles/units/heroes/hero_invoker/invoker_cold_snap_status.vpcf" end
		function modifier_invoker_lua_cold_snap_buff:DeclareFunctions()
			local funcs = {
		...
		function modifier_invoker_lua_cold_snap_buff:OnAttackLanded(kv)
			if IsServer() then 
				...
				if target == self:GetParent() and attacker:GetTeam() ~= self:GetCaster():GetTeam() and attacker ~= self:GetParent() and not attacker:IsOther() and not attacker:IsBuilding() and not attacker:IsMagicImmune() then
					attacker:AddNewModifier(self.caster, self.ability, "modifier_invoker_lua_cold_snap_stun_duration", {duration = self.freeze_duration * (1 - attacker:GetStatusResistance())}) 
		...
		function modifier_invoker_lua_cold_snap_buff:OnCreated(kv) 
			if IsServer() then
				self.caster 			= self:GetCaster()
				self.ability 			= self:GetAbility()
				self.quas_level 		= self.caster:FindAbilityByName("invoker_lua_quas"):GetLevel() - 1
	...
	---------------------------------------------------------------------------------------------------------------------
		LinkLuaModifier("modifier_invoker_lua_ghost_walk", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)

		invoker_lua_ghost_walk = class({})

		function invoker_lua_ghost_walk:GetCastAnimation()
			return ACT_DOTA_CAST_GHOST_WALK
		end

		function invoker_lua_ghost_walk:OnSpellStart()
			if IsServer() then
				local caster 						= self:GetCaster()
				local ability 						= caster:FindAbilityByName("invoker_lua_ghost_walk")
				...
				local quas_level 					= caster:FindAbilityByName("invoker_lua_quas"):GetLevel() - 1
				local wex_level 					= caster:FindAbilityByName("invoker_lua_wex"):GetLevel() - 1
				...
				caster:AddNewModifier(caster, self, "modifier_invoker_lua_ghost_walk", {	duration 			= ghost_walk_duration, 
																							self_slow 			= self_slow, 
		...
		---------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_ghost_walk = class({})
		LinkLuaModifier("modifier_invis_fade_ghost_walk", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_ghost_walk_aura", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		function modifier_invoker_lua_ghost_walk:IsHidden() 		return false end
		function modifier_invoker_lua_ghost_walk:IsPurgable() 		return false end
		function modifier_invoker_lua_ghost_walk:IsDebuff() 		return false end
		function modifier_invoker_lua_ghost_walk:GetEffectName() 	return "particles/units/heroes/hero_invoker/invoker_ghost_walk.vpcf" end
		function modifier_invoker_lua_ghost_walk:DeclareFunctions()
			local funcs = {
				MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
		...
		function modifier_invoker_lua_ghost_walk:GetModifierMoveSpeed_Max()
			return self.max_movement_speed
		end

		function modifier_invoker_lua_ghost_walk:CheckState()
			local state = {
		...
		function modifier_invoker_lua_ghost_walk:GetPriority()
			return MODIFIER_PRIORITY_NORMAL
		end

		function modifier_invoker_lua_ghost_walk:GetModifierInvisibilityLevel()
			return 100
		end

		function modifier_invoker_lua_ghost_walk:OnCreated(kv) 
			if IsServer() then 
				self.caster 				= self:GetCaster()
		...
		function modifier_invoker_lua_ghost_walk:OnRefresh(kv) 
			if IsServer() then
				self.area_of_effect 		= kv.area_of_effect
				self.aura_fade_time 		= kv.aura_fade_time
				self.enemy_slow 			= kv.enemy_slow
		...
		-- function modifier_invoker_lua_ghost_walk:OnIntervalThink()
			-- if IsServer() then
				-- local nearby_enemy_units = FindUnitsInRadius(	
					-- self.caster:GetTeam(), 
				...
				-- if nearby_enemy_units ~= nil then
					-- for _,enemy in pairs(nearby_enemy_units) do
						-- if enemy ~= self.parent then
							-- enemy:AddNewModifier(self.caster, self:GetAbility(), "modifier_invoker_lua_ghost_walk_aura", {duration = self.aura_fade_time, enemy_slow = self.enemy_slow})
						-- end
					-- end
				-- end
			-- end
		...
		function modifier_invoker_lua_ghost_walk:OnAttack(kv)
			if IsServer() then	
				-- Only break ghost_walk if invoker is attacking
				if self.parent == kv.attacker then
					if self.caster:HasModifier("modifier_invoker_lua_ghost_walk") then
						self.caster:RemoveModifierByName("modifier_invoker_lua_ghost_walk")
					end
				end
			end
		end

		function modifier_invoker_lua_ghost_walk:OnAbilityExecuted(kv)
			if IsServer() then
				local caster = kv.unit
				-- Only break ghost_walk if invoker executed an ability
				if self.parent == caster then 
					if caster:HasModifier("modifier_invoker_lua_ghost_walk") then
						self.caster:RemoveModifierByName("modifier_invoker_lua_ghost_walk")
		...
		function modifier_invoker_lua_ghost_walk:GetModifierMoveSpeedBonus_Percentage()
			return self.self_slow
		end
		
		function modifier_invoker_lua_ghost_walk:IsAura() 					return true end
		function modifier_invoker_lua_ghost_walk:GetModifierAura()			return "modifier_invoker_lua_ghost_walk_aura" end

		function modifier_invoker_lua_ghost_walk:GetAuraRadius()			return self.area_of_effect end
		function modifier_invoker_lua_ghost_walk:GetAuraSearchFlags()		return DOTA_UNIT_TARGET_FLAG_NONE end
		function modifier_invoker_lua_ghost_walk:GetAuraSearchTeam()		return DOTA_UNIT_TARGET_TEAM_ENEMY end
		function modifier_invoker_lua_ghost_walk:GetAuraSearchType()		return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end

		function modifier_invoker_lua_ghost_walk:IsAuraActiveOnDeath() 	return false end
		function modifier_invoker_lua_ghost_walk:GetAuraDuration()			return self.aura_fade_time end
		...
		--	Ghost Walk - SlowAura
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_ghost_walk_aura = class({})
		function modifier_invoker_lua_ghost_walk_aura:IsHidden() 		return false end
		function modifier_invoker_lua_ghost_walk_aura:IsPurgable() 	return false end
		function modifier_invoker_lua_ghost_walk_aura:IsDebuff() 		return true  end
		function modifier_invoker_lua_ghost_walk_aura:GetEffectName() 	return "particles/units/heroes/hero_invoker/invoker_ghost_walk_debuff.vpcf" end
		function modifier_invoker_lua_ghost_walk_aura:DeclareFunctions()
			local funcs = {
				MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE
			}
		...
		function modifier_invoker_lua_ghost_walk_aura:OnCreated(kv)
			if IsServer() then
				self.caster 		= self:GetCaster()
				self.parent 		= self:GetParent()
				...
				local quas_level 					= self.caster:FindAbilityByName("invoker_lua_quas"):GetLevel() - 1
		...
		-- function modifier_invoker_lua_ghost_walk_aura:OnRefresh(kv) 
			-- if IsServer() then
				-- self.enemy_slow = kv.enemy_slow
			-- else
				-- local net_table = CustomNetTables:GetTableValue("player_table", tostring(self:GetCaster():GetPlayerOwnerID())) or {}
		...
		function modifier_invoker_lua_ghost_walk_aura:OnRemoved()
			if IsServer() then	
		...
		function modifier_invoker_lua_ghost_walk_aura:GetModifierMoveSpeedBonus_Percentage()
	...
	--------------------------------------------------------------------------------------------------------------------
		...
		function invoker_lua_alacrity:OnSpellStart()
			if IsServer() then
				local caster 				= self:GetCaster()
				local target 				= self:GetCursorTarget() 
				local ability 				= self
				...
				-- Get skill stats
				local wex_level 			= caster:FindAbilityByName("invoker_lua_wex"):GetLevel() - 1
				local exort_level 			= caster:FindAbilityByName("invoker_lua_exort"):GetLevel() - 1
				...
				if caster:HasTalent("imba_special_bonus_unique_invoker_6") and target == caster then 
					for _,hero in pairs(HeroList:GetAllHeroes()) do
						if hero == caster then 
							hero:AddNewModifier(caster, ability, "modifier_invoker_lua_alacrity", {
					...
					target:AddNewModifier(caster, ability, "modifier_invoker_lua_alacrity", {
						duration = alacrity_duration, 
						bonus_attack_speed = bonus_attack_speed, 
						bonus_damage = bonus_damage})
				end
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_alacrity = class({})
		function modifier_invoker_lua_alacrity:IsHidden() 		return false end
		function modifier_invoker_lua_alacrity:IsPurgable() 	return false end
		function modifier_invoker_lua_alacrity:IsBuff() 		return true  end
		function modifier_invoker_lua_alacrity:GetEffectName() return invoker_lua_alacrity.ability_effect_path end
		function modifier_invoker_lua_alacrity:DeclareFunctions()
			local funcs = {
				MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT,
		...
		function modifier_invoker_lua_alacrity:OnCreated(kv)
			if IsServer() then
				self.parent 			= self:GetParent()
				self.ability 			= self:GetAbility()
				self.number_of_chains 	= self.ability:GetSpecialValueFor("number_of_chains")

				self.alacrity_effect 	= ParticleManager:CreateParticle(invoker_lua_alacrity.buff_effect_path, PATTACH_OVERHEAD_FOLLOW, self.parent)
		...
		function modifier_invoker_lua_alacrity:OnAttackLanded(kv)
			if IsServer() then
				...
				if kv.attacker == parent and ( not parent:IsIllusion() ) then
					...
					if parent:IsRangedAttacker() then 
						modifier_invoker_lua_alacrity:Ranged_Chain_Attack(caster, parent, self.ability, kv.target, kv.damage, self.number_of_chains) 
					else
						modifier_invoker_lua_alacrity:Melee_Chain_lightning(caster, parent, self.ability, kv.target, kv.damage, self.number_of_chains) 			
					end
				end
			end
		end

		function modifier_invoker_lua_alacrity:Ranged_Chain_Attack(caster, attacker, ability, target, damage, chains) 
			if IsServer() then 
		...
		function invoker_lua_alacrity:OnProjectileHit_ExtraData(target, location, ExtraData) 
			if IsServer() then 
				local ability 			= self
		...
		function modifier_invoker_lua_alacrity:Melee_Chain_lightning(caster, attacker, ability, target, damage, chains) 
			if IsServer() then 
				...
				if chains ~= nil and chains > 0 then
					...
					for _,enemy in pairs(chain_enemies) do
						if enemy ~= target then 
							...
							chains = chains - 1
							modifier_invoker_lua_alacrity:Melee_Chain_lightning(caster, attacker, ability, enemy, damage, chains) 
							return
						end
					end
				end
		...
		function modifier_invoker_lua_alacrity:OnRemoved()
			if IsServer() then
		...
		function modifier_invoker_lua_alacrity:OnRefresh(kv)
			if IsServer() then
				self.bonus_damage 		= kv.bonus_damage
				self.bonus_attack_speed = kv.bonus_attack_speed
			else
		...
		function modifier_invoker_lua_alacrity:GetModifierAttackSpeedBonus_Constant()
			return self.bonus_attack_speed 
		end

		function modifier_invoker_lua_alacrity:GetModifierPreAttack_BonusDamage()
	...
	--------------------------------------------------------------------------------------------------------------------
		LinkLuaModifier("modifier_invoker_lua_forge_spirit", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)

		invoker_lua_forge_spirit = class({})

		function invoker_lua_forge_spirit:GetCastAnimation()
			return ACT_DOTA_CAST_FORGE_SPIRIT
		end

		function invoker_lua_forge_spirit:OnSpellStart()
			if IsServer() then
				local caster 		= self:GetCaster()
				local ability 		= caster:FindAbilityByName("invoker_lua_forge_spirit")
				local quas_level 	= caster:FindAbilityByName("invoker_lua_quas"):GetLevel() - 1
				local exort_level 	= caster:FindAbilityByName("invoker_lua_exort"):GetLevel() - 1
				...
				for i = 1, spirit_count do
					...
					forged_spirit:AddNewModifier(
						caster, 
						self, 
						"modifier_invoker_lua_forge_spirit", 
						{	
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_forge_spirit = class({})
		LinkLuaModifier("modifier_imba_forged_spirit_melting_strike", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		function modifier_invoker_lua_forge_spirit:IsHidden() 	return true end
		function modifier_invoker_lua_forge_spirit:IsPassive() return true end
		function modifier_invoker_lua_forge_spirit:DeclareFunctions()
			local funcs = {
		...
		function modifier_invoker_lua_forge_spirit:OnCreated(kv)
			if IsServer() then
				self.caster					= self:GetCaster()
				self.ability 				= self:GetAbility()
				self.parent 				= self:GetParent()
				local quas_level 			= self.caster:FindAbilityByName("invoker_lua_quas"):GetLevel() - 1
				local exort_level 			= self.caster:FindAbilityByName("invoker_lua_exort"):GetLevel() - 1
		...
		function modifier_invoker_lua_forge_spirit:OnAttackLanded(kv)
			if IsServer() then
				local attacker = kv.attacker
				local target = kv.target
		...
		function modifier_invoker_lua_forge_spirit:GetModifierAttackRangeBonus()
			return self:GetStackCount() 
		end

		function modifier_invoker_lua_forge_spirit:OnRemoved()
			if IsServer() then 
	...
	--	Invoker's: Tornado
	--------------------------------------------------------------------------------------------------------------------
		invoker_lua_tornado = class({})
		invoker_lua_tornado.loop_interval = 0.03
		invoker_lua_tornado.ability_effect_path 			= "particles/units/heroes/hero_invoker/invoker_tornado.vpcf"
--		invoker_lua_tornado.ability_effect_path 			= "particles/econ/items/invoker/invoker_ti6/invoker_tornado_ti6.vpcf"
		invoker_lua_tornado.ability_effect_cyclone_path 	= "particles/units/heroes/hero_invoker/invoker_tornado_child.vpcf"
--		invoker_lua_tornado.ability_effect_cyclone_path 	= "particles/econ/items/invoker/invoker_ti6/invoker_tornado_child_ti6.vpcf"
		LinkLuaModifier("modifier_invoker_lua_tornado", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_tornado_cyclone", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_tornado_empower_debuff", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		function invoker_lua_tornado:GetCooldown(invoke_level)
		...
		function invoker_lua_tornado:GetCastAnimation()
			return ACT_DOTA_CAST_TORNADO
		end

		function invoker_lua_tornado:OnSpellStart()
			if IsServer() then 
				local caster 					= self:GetCaster()
				local ability 					= caster:FindAbilityByName("invoker_lua_tornado")
				...
				{
					EffectName 			= invoker_lua_tornado.ability_effect_path,
					Ability 			= ability,
					...
					ExtraData 			= 	{	tornado_lift_duration 	= tornado_lift_duration, 
												...
												cyclone_effect_path 	= invoker_lua_tornado.ability_effect_cyclone_path,
												vision_distance 		= vision_distance,
		...
		function invoker_lua_tornado:OnProjectileThink_ExtraData(vLocation, ExtraData)
			if IsServer() then 
		...
		--------------------------------------------------------------------------------------------------------------------
		function invoker_lua_tornado:OnProjectileHit_ExtraData(target, location, ExtraData)
			if IsServer() then
				if target ~= nil then 
					local caster 					= self:GetCaster()
					...
					if tornado_lift_duration ~= nil then
						...
						target:AddNewModifier(caster, target, "modifier_invoker_lua_tornado_cyclone", {duration = tornado_lift_duration * (1 - target:GetStatusResistance())})
						...
						local flying_z_modifier = target:FindModifierByName("modifier_invoker_lua_tornado_cyclone")
						local z_position = 0
						...
						local initial_ascent_height_per_frame 	= ((cyclone_initial_height) / time_to_reach_initial_height) * invoker_lua_tornado.loop_interval 
						...
						local time_to_stop_fly = tornado_lift_duration - time_to_reach_initial_height -- invoker_lua_tornado.loop_interval
						-- Loop up and down
						local going_up = true
						...
						Timers:CreateTimer(function()
							...
							if z_position < cyclone_initial_height and time_in_air <= time_to_reach_initial_height then
								...
								return invoker_lua_tornado.loop_interval
							...
							elseif time_in_air > time_to_stop_fly and time_in_air <= tornado_lift_duration then
								...
								if final_descent_height_per_frame == nil then
									...
									-- Since (time_to_reach_initial_height / invoker_lua_tornado.loop_interval) will end up in a decimal we will get rounding issues in the loop. 
									-- By using math.floor we discards decimals and gives us the correct value which to divide reamining distance to ground
									local rounding_coeff = math.floor(time_to_reach_initial_height / invoker_lua_tornado.loop_interval)
							...
							elseif time_in_air <= tornado_lift_duration then
								...
								if target:HasModifier("modifier_invoker_lua_tornado") then
									target:RemoveModifierByName("modifier_invoker_lua_tornado") 
								end

								if target:HasModifier("modifier_invoker_lua_tornado_cyclone") then
									target:RemoveModifierByName("modifier_invoker_lua_tornado_cyclone")
								...
								if ExtraData.daze_duration > 0 then
									target:AddNewModifier(caster, self, "modifier_invoker_lua_tornado_empower_debuff", {duration = ExtraData.daze_duration * (1 - target:GetStatusResistance())})
								end
						end)		
					end
				else 
		...
		function invoker_lua_tornado:spinn(kv)
			local target = kv.target
			...
			if kv.target.invoker_tornado_degrees_to_spin == nil and kv.tornado_lift_duration ~= nil then
				...
				kv.target.invoker_tornado_degrees_to_spin = degrees_per_second_ending_in_same_forward_vector * invoker_lua_tornado.loop_interval
			end
		...
		function invoker_lua_tornado:LandDamage(target, attacker, ability, kv)
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_tornado = class({})
		function modifier_invoker_lua_tornado:IsHidden() 	return false end
		function modifier_invoker_lua_tornado:IsBuff() 	return false end
		function modifier_invoker_lua_tornado:IsDebuff() 	return false end
		function modifier_invoker_lua_tornado:IsPassive() 	return false end
		function modifier_invoker_lua_tornado:IsPurgable() return false end
		function modifier_invoker_lua_tornado:CheckState()
			local state = {
				[MODIFIER_STATE_NO_UNIT_COLLISION] 	= true,
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_tornado_cyclone = class({})
		function modifier_invoker_lua_tornado_cyclone:IsHidden() 	return true  end
		function modifier_invoker_lua_tornado_cyclone:IsBuff() 	return false end
		function modifier_invoker_lua_tornado_cyclone:IsDebuff() 	return false end
		function modifier_invoker_lua_tornado_cyclone:IsPassive() 	return false end
		function modifier_invoker_lua_tornado_cyclone:IsPurgable() return false end
		function modifier_invoker_lua_tornado_cyclone:CheckState()
			local state = {
				[MODIFIER_STATE_NO_UNIT_COLLISION] 	= true,
				[MODIFIER_STATE_STUNNED] 			= true,
				[MODIFIER_STATE_ROOTED] 			= true,
		...
		function modifier_invoker_lua_tornado_cyclone:DeclareFunctions()
			local funcs = {
		...
		function modifier_invoker_lua_tornado_cyclone:GetVisualZDelta()
			return self:GetStackCount()
		end

		function modifier_invoker_lua_tornado_cyclone:OnCreated(kv)
		end


		function modifier_invoker_lua_tornado_cyclone:OnRemoved(kv)
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_tornado_empower_debuff = class({})
		function modifier_invoker_lua_tornado_empower_debuff:IsHidden() 	return false end
		function modifier_invoker_lua_tornado_empower_debuff:IsBuff() 		return false end
		function modifier_invoker_lua_tornado_empower_debuff:IsDebuff() 	return true end
		function modifier_invoker_lua_tornado_empower_debuff:IsPassive() 	return false end
		function modifier_invoker_lua_tornado_empower_debuff:IsPurgable() 	return false end
		function modifier_invoker_lua_tornado_empower_debuff:CheckState()
			local state = {
	...
	--	Invoker's EMP
	---------------------------------------------------------------------------------------------------------------------
		invoker_lua_emp = class({})
		LinkLuaModifier("modifier_invoker_lua_emp", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_emp_overload", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)

		function invoker_lua_emp:GetCastAnimation()
			return ACT_DOTA_CAST_EMP
		end

		function invoker_lua_emp:GetAOERadius()
			return self:GetSpecialValueFor("area_of_effect")
		end 

		function invoker_lua_emp:OnSpellStart()
			 if IsServer() then 
		...
		function invoker_lua_emp:CastEMP(kv)	
			local caster 			= kv.caster
			local ability 			= kv.ability
			local target_point 		= kv.target_point
		...
		--	EMP ThinkerModifier
		-----------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_emp = class({})
		function modifier_invoker_lua_emp:OnCreated(kv)
			if IsServer() then 
				self.caster 					= self:GetCaster()
				self.ability 					= self:GetAbility()
				self.target_point 				= self:GetParent():GetAbsOrigin()
				...
				local invoker_wex 				= self.caster:FindAbilityByName("invoker_lua_wex")
		...
		function modifier_invoker_lua_emp:OnRemoved()
			if IsServer() then
				if self.emp_effect then
		...
		function modifier_invoker_lua_emp:OnHit(caster, ability, enemy, mana_to_burn, after_shock_duration, damage_per_mana_pct, mana_gain_per_mana_pct)
			...
			if enemy:IsRealHero() then
				enemy:AddNewModifier(caster, ability, "modifier_invoker_lua_emp_overload", {duration = after_shock_duration * (1 - enemy:GetStatusResistance())}) 
		...
		-----------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_emp_overload = class({})
		function modifier_invoker_lua_emp_overload:IsPassive() 	return false end
		function modifier_invoker_lua_emp_overload:IsBuff() 		return false end
		function modifier_invoker_lua_emp_overload:IsDebuff() 		return true  end
		function modifier_invoker_lua_emp_overload:IsPurgable() 	return true  end
		function modifier_invoker_lua_emp_overload:IsHidden() 		return false end
		function modifier_invoker_lua_emp_overload:DeclareFunctions()
			local funcs = {
		...
		function modifier_invoker_lua_emp_overload:OnManaGained(kv) 
	...
	--	Invoker's: Ice Wall
	--------------------------------------------------------------------------------------------------------------------
		invoker_lua_ice_wall = class({})
		invoker_lua_ice_wall.ice_wall_effect = "particles/units/heroes/hero_invoker/invoker_ice_wall.vpcf"
		LinkLuaModifier("modifier_invoker_lua_ice_wall", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_ice_wall_slow", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_ice_wall_attack_slow", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		function invoker_lua_ice_wall:GetCastAnimation()
			return ACT_DOTA_CAST_ICE_WALL
		end

		function invoker_lua_ice_wall:OnSpellStart()
			if IsServer() then
				local caster				 		= self:GetCaster()
				local caster_point 					= caster:GetAbsOrigin() 
				local caster_direction 				= caster:GetForwardVector()
				local ability 						= caster:FindAbilityByName("invoker_lua_ice_wall")
				local quas_level 					= caster:FindAbilityByName("invoker_lua_quas"):GetLevel() - 1
				local exort_level 					= caster:FindAbilityByName("invoker_lua_exort"):GetLevel() - 1
				...
				for i = 0, (ice_walls -1) do 
					...
					local ice_wall_particle_effect = ParticleManager:CreateParticle(invoker_lua_ice_wall.ice_wall_effect, PATTACH_CUSTOMORIGIN, nil)
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_ice_wall = class({})
		modifier_invoker_lua_ice_wall.npc_radius_constant = 65
		function modifier_invoker_lua_ice_wall:OnCreated(kv)
			if IsServer() then
		...
		function modifier_invoker_lua_ice_wall:OnIntervalThink()
			if IsServer() then
				local nearby_enemy_units = FindUnitsInRadius(
					self.GetTeam, 
					self.origin, 
				...
				for _,enemy in pairs(nearby_enemy_units) do
					if enemy ~= nil and enemy:IsAlive() then
						...
						if self:IsUnitInProximity(self.ice_wall_start_point, self.ice_wall_end_point, target_position, self.ice_wall_area_of_effect) then
							enemy:AddNewModifier(self:GetCaster(), self.ability, "modifier_invoker_lua_ice_wall_slow", {duration = self.slow_duration, enemy_slow = self.ice_wall_slow * (1 - enemy:GetStatusResistance())})
							enemy:AddNewModifier(self:GetCaster(), self.ability, "modifier_invoker_lua_ice_wall_attack_slow", {duration = self.slow_duration, enemy_slow = self.attack_slow * (1 - enemy:GetStatusResistance())})
		...
		function modifier_invoker_lua_ice_wall:OnRemoved() 
		...
		--------------------------------------------------------------------------------------------------------------------
		function modifier_invoker_lua_ice_wall:IsUnitInProximity(start_point, end_point, target_position, ice_wall_radius)
			...
			return distance:Length2D() <= ice_wall_radius + modifier_invoker_lua_ice_wall.npc_radius_constant
		end

		--------------------------------------------------------------------------------------------------------------------
		--	Invoker's: Ice Wall slow aura 
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_ice_wall_slow = class({})
		function modifier_invoker_lua_ice_wall_slow:IsPassive() 		return false end
		function modifier_invoker_lua_ice_wall_slow:IsBuff() 			return false end
		function modifier_invoker_lua_ice_wall_slow:IsDebuff() 		return true  end
		function modifier_invoker_lua_ice_wall_slow:IsPurgable() 		return false end
		function modifier_invoker_lua_ice_wall_slow:IsHidden() 		return false end
		function modifier_invoker_lua_ice_wall_slow:GetEffectName() 	return "particles/units/heroes/hero_invoker/invoker_ice_wall_debuff.vpcf" end
		function modifier_invoker_lua_ice_wall_slow:DeclareFunctions()
			local funcs = {
				MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE
			}
		...
		function modifier_invoker_lua_ice_wall_slow:GetTexture()
			return "invoker_ice_wall"
		end

		function modifier_invoker_lua_ice_wall_slow:OnCreated(kv)
			if IsServer() then
				self.caster = self:GetCaster()
				self.parent = self:GetParent()
		...
		function modifier_invoker_lua_ice_wall_slow:OnRefresh(kv) 
			if IsServer() then
				self:SetStackCount(kv.enemy_slow)
			end
		end

		function modifier_invoker_lua_ice_wall_slow:OnRemoved()
			if IsServer() then
		...
		function modifier_invoker_lua_ice_wall_slow:GetModifierMoveSpeedBonus_Percentage()
			return self:GetStackCount()
		end
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_ice_wall_attack_slow = class({})
		function modifier_invoker_lua_ice_wall_attack_slow:IsDebuff()			return true  end
		function modifier_invoker_lua_ice_wall_attack_slow:IsHidden() 			return true  end
		function modifier_invoker_lua_ice_wall_attack_slow:IsPurgable() 		return true  end
		function modifier_invoker_lua_ice_wall_attack_slow:IsPurgeException() 	return true  end
		function modifier_invoker_lua_ice_wall_attack_slow:IsStunDebuff() 		return false end
		function modifier_invoker_lua_ice_wall_attack_slow:DeclareFunctions()
			local funcs = {
				MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT
			}
		...
		function modifier_invoker_lua_ice_wall_attack_slow:GetTexture()
			return "invoker_ice_wall"
		end

		function modifier_invoker_lua_ice_wall_attack_slow:OnCreated(kv)
			if IsServer() then
				self.caster = self:GetCaster()
				self.parent = self:GetParent()
				self:SetStackCount(kv.enemy_slow)
		...
		function modifier_invoker_lua_ice_wall_attack_slow:OnRefresh(kv) 
			if IsServer() then
				self:SetStackCount(kv.enemy_slow)
			end
		end

		function modifier_invoker_lua_ice_wall_attack_slow:GetModifierMoveSpeedBonus_Percentage()
			return self:GetStackCount()
		end
	...
	--------------------------------------------------------------------------------------------------------------------
		invoker_lua_chaos_meteor = class({})
		invoker_lua_chaos_meteor.loop_interval = 0.03
		LinkLuaModifier("modifier_invoker_lua_chaos_meteor", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_chaos_meteor_burn", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_chaos_meteor_burn_effect", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_chaos_meteor_aura", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		function invoker_lua_chaos_meteor:GetCastAnimation()
			return ACT_DOTA_CAST_CHAOS_METEOR
		end

		function invoker_lua_chaos_meteor:OnSpellStart()
			if IsServer() then
				local caster 			= self:GetCaster()
				local ability 			= self
				local target_point 		= self:GetCursorPosition()
				...
				invoker_lua_chaos_meteor:CastMeteor(caster, ability, target_point, number_of_meteors)
				...
				if number_of_meteors > 1 then 
					...
					Timers:CreateTimer({
						...
						callback = function()
							...
							invoker_lua_chaos_meteor:CastMeteor(caster, ability, new_target_point, number_of_meteors)
		...
		function invoker_lua_chaos_meteor:CastMeteor(caster, ability, target_point, number_of_meteors)
			if IsServer() then 
				...
				CreateModifierThinker(	
					caster, 
					ability, 
					"modifier_invoker_lua_chaos_meteor", 	
					{
		...
		modifier_invoker_lua_chaos_meteor = class({})
		function modifier_invoker_lua_chaos_meteor:OnCreated(kv)
			if IsServer() then
				self.caster = self:GetCaster()
				self.ability = self:GetAbility()
				self.target_point = self:GetParent():GetAbsOrigin()
		...
		function invoker_lua_chaos_meteor:OnProjectileThink_ExtraData(location, ExtraData)
			if IsServer() then
				EntIndexToHScript(ExtraData.meteor_dummy):SetAbsOrigin(location)
			end
		end

		function invoker_lua_chaos_meteor:OnProjectileHit_ExtraData(target, location, ExtraData)
			if IsServer() then 
				if target == nil then 
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_chaos_meteor_aura = class({})
		function modifier_invoker_lua_chaos_meteor_aura:OnCreated(kv)
			if IsServer() then
				self.caster 				= self:GetCaster()
				self.ability 				= self:GetAbility()
				self.GetTeam 				= self:GetParent():GetTeam()
		...
		function modifier_invoker_lua_chaos_meteor_aura:OnIntervalThink()
			if IsServer() then
				...
				for _,enemy in pairs(self.hit_table) do 
					if enemy:IsNull() == false and enemy:HasModifier("modifier_invoker_lua_chaos_meteor_burn") then 
						-- Reusing old method for calculating distance vs a line
						if modifier_invoker_lua_ice_wall:IsUnitInProximity(start_point, end_point, enemy:GetAbsOrigin(), 300) then
							local burn_modifiers = enemy:FindAllModifiersByName("modifier_invoker_lua_chaos_meteor_burn")
							for _,modifier in pairs(burn_modifiers) do
								modifier:ForceRefresh()
							end

							local burn_effect_modifier = enemy:FindModifierByName("modifier_invoker_lua_chaos_meteor_burn_effect")
				...
				for _,enemy in pairs(nearby_enemy_units) do
					...
					if enemy ~= nil then
						...
						enemy:AddNewModifier(	
							self.caster, 
							self.ability, 
							"modifier_invoker_lua_chaos_meteor_burn", 
							{
						...
						enemy:AddNewModifier(	
							self.caster, 
							self.ability, 
							"modifier_invoker_lua_chaos_meteor_burn_effect", 
							{
								duration 			= self.burn_duration
							}
						)
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_chaos_meteor_burn = class({})
		function modifier_invoker_lua_chaos_meteor_burn:IsDebuff() return true end
		function modifier_invoker_lua_chaos_meteor_burn:IgnoreTenacity()	return true end
		function modifier_invoker_lua_chaos_meteor_burn:GetAttributes()
			return MODIFIER_ATTRIBUTE_MULTIPLE
		end

		function modifier_invoker_lua_chaos_meteor_burn:OnCreated(kv) 
			if IsServer() then
				self.caster 			= self:GetCaster()
				self.parent 			= self:GetParent()
				self.ability 			= self:GetAbility()
		...
		function modifier_invoker_lua_chaos_meteor_burn:OnIntervalThink()
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_chaos_meteor_burn_effect = class({})
...
--		invoker_lua_deafening_blast.ability_effect_path 			= "particles/econ/items/invoker/invoker_ti6/invoker_deafening_blast_ti6.vpcf"
		...
		LinkLuaModifier("modifier_invoker_lua_deafening_blast", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_deafening_blast_frozen", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_deafening_blast_knockback", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)
		LinkLuaModifier("modifier_invoker_lua_deafening_blast_disarm", "pathfinder/invoker/invoker_lua.lua", LUA_MODIFIER_MOTION_NONE)

		function invoker_lua_deafening_blast:GetCastAnimation()
			return ACT_DOTA_CAST_DEAFENING_BLAST
		end

		function invoker_lua_deafening_blast:OnSpellStart()
			if IsServer() then 
				local caster 			= self:GetCaster()
				local ability 			= self
				local caster_location 	= caster:GetAbsOrigin()
				...
				if caster:HasTalent("imba_special_bonus_unique_invoker_7") then
					...
					CreateModifierThinker(
							caster, 
							ability, 
							"modifier_invoker_lua_deafening_blast", 	
							{
				...
				-- Cast normal
					...
					invoker_lua_deafening_blast:CastDeafeningBlast(caster, ability, target_point)
				end
			end
		end

		function invoker_lua_deafening_blast:CastDeafeningBlast(caster, ability, target_point)
			if IsServer() then
				local caster_location 	= caster:GetAbsOrigin()
				local quas_level 		= caster:FindAbilityByName("invoker_lua_quas"):GetLevel() - 1
				local exort_level 		= caster:FindAbilityByName("invoker_lua_exort"):GetLevel() - 1
				...
				{
					EffectName 			= invoker_lua_deafening_blast.ability_effect_path,
					Ability 			= ability,
					...
					ExtraData = {
						...
						disarm_effect_path 					= invoker_lua_deafening_blast.ability_disarm_effect_path,
						knockback_effect_path 				= invoker_lua_deafening_blast.ability_knockback_effect_path
					}
				}
		...
		function invoker_lua_deafening_blast:RadialDeafeningBlast(caster, ability, target_point, num_blasts)
			if IsServer() then 
				...
				for index = 1, num_blasts, 1 do
					...
					local deafening_blast_projectile_table = 
					{
						EffectName 			= invoker_lua_deafening_blast.ability_effect_path_aoe,
						Ability 			= ability,
						vSpawnOrigin 		= caster:GetAbsOrigin(),
						vVelocity 			= direction * deafening_blast_travel_speed,
						fDistance 			= deafening_blast_travel_distance,
		...
		function invoker_lua_deafening_blast:OnProjectileHit_ExtraData(target, location, ExtraData)
			if IsServer() then
				if target then
					local caster = self:GetCaster()
		...
		function invoker_lua_deafening_blast:CheckHitList(target_entity_id) 
		...
		function invoker_lua_deafening_blast:KnockBackAndDisarm(caster, target, knockback_duration, disarm_duration, disarm_effect_path, knockback_effect_path)
			-- Add knockback and frozen modifier
			target:AddNewModifier(caster, self, "modifier_invoker_lua_deafening_blast_knockback", {duration = knockback_duration * (1 - target:GetStatusResistance())})
			target:AddNewModifier(caster, self, "modifier_invoker_lua_deafening_blast_frozen", {duration = knockback_duration + disarm_duration * (1 - target:GetStatusResistance())})
		end

		modifier_invoker_lua_deafening_blast = class({})
		function modifier_invoker_lua_deafening_blast:OnCreated(kv)
			if IsServer() then
				self.target_point 		= self:GetParent():GetAbsOrigin()
		...
		function modifier_invoker_lua_deafening_blast:OnIntervalThink()
			if IsServer() then 
				-- Create projectile
				local deafening_blast_projectile_table = 
				{
				...
				if self.remaining_deafening_blast == 0 then 
					self:GetParent():RemoveModifierByName("modifier_invoker_lua_deafening_blast")
				end
			end
		end

		function modifier_invoker_lua_deafening_blast:OnRemoved()
			if IsServer() then
				invoker_lua_deafening_blast:RadialDeafeningBlast(
					self.caster, 
					self.ability,
					self.target_point,
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_deafening_blast_frozen = class({})
		function modifier_invoker_lua_deafening_blast_frozen:IsHidden() 		return false end
		function modifier_invoker_lua_deafening_blast_frozen:IsDebuff() 		return true  end
		function modifier_invoker_lua_deafening_blast_frozen:IsPurgable() 		return false end
		function modifier_invoker_lua_deafening_blast_frozen:GetEffectName() 	return "particles/status_fx/status_effect_frost.vpcf" end
		function modifier_invoker_lua_deafening_blast_frozen:CheckState()
			local state = {
				[MODIFIER_STATE_FROZEN] = true
			}
			return state
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_deafening_blast_knockback = class({})
		function modifier_invoker_lua_deafening_blast_knockback:IsHidden() 	return false end
		function modifier_invoker_lua_deafening_blast_knockback:IsDebuff() 	return true  end
		function modifier_invoker_lua_deafening_blast_knockback:IsPurgable() 	return false end
		function modifier_invoker_lua_deafening_blast_knockback:CheckState()
		local state = {
				[MODIFIER_STATE_ROOTED] = true,
			}
			return state
		...
		function modifier_invoker_lua_deafening_blast_knockback:GetOverrideAnimation()
			return ACT_DOTA_DISABLED
		end

		function modifier_invoker_lua_deafening_blast_knockback:OnCreated(kv)
			if IsServer() then 
				...
				self:StartIntervalThink(invoker_lua_deafening_blast.knockback_interval)
			end
		end

		function modifier_invoker_lua_deafening_blast_knockback:OnIntervalThink()
			if IsServer() then
		...
		function modifier_invoker_lua_deafening_blast_knockback:OnRemoved()
			if IsServer() then
		...
		--------------------------------------------------------------------------------------------------------------------
		modifier_invoker_lua_deafening_blast_disarm = class({})
		function modifier_invoker_lua_deafening_blast_disarm:IsHidden() 	return true end
		function modifier_invoker_lua_deafening_blast_disarm:IsDebuff() 	return true end
		function modifier_invoker_lua_deafening_blast_disarm:IsPurgable() 	return false end
		function modifier_invoker_lua_deafening_blast_disarm:StatusEffectPriority() return 15 end
		function modifier_invoker_lua_deafening_blast_disarm:CheckState()
			local state = {
				[MODIFIER_STATE_DISARMED] = true
			}
			return state
