...
function modifier_pangolier_npc_gyroshell_lua:OnCreated()
	...
	if IsServer() then
        ...
        self:GetParent():AddNewModifier(self:GetCaster(), self, "modifier_npc_gyroshell_impact_check", {duration = 5 })
...
function modifier_pangolier_npc_gyroshell_lua:IsMotionController() return true end
function modifier_pangolier_npc_gyroshell_lua:GetMotionControllerPriority() return DOTA_MOTION_CONTROLLER_PRIORITY_MEDIUM end

function modifier_pangolier_npc_gyroshell_lua:OnIntervalThink()
	--Interrupt if Pangolier has been stunned, rooted or taunted
	if self:GetParent():IsStunned() or self:GetParent():IsRooted() or self:GetParent():GetForceAttackTarget() then
		 return self:Destroy()
	end
...
function modifier_npc_gyroshell_impacted:OnCreated()
