...
function trap_catapult_attack:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		if hTarget ~= nil and ( not hTarget:IsMagicImmune() ) and ( not hTarget:IsInvulnerable() ) then
			...
			hTarget:AddNewModifier( self:GetCaster(), self, "modifier_rooted", { duration = self:GetSpecialValueFor( "root_duration" ) } )

			local damage_percent = self:GetLevelSpecialValueFor("damage_percent", (self:GetLevel() - 1))
			local damage = hTarget:GetMaxHealth()/ 100 * damage_percent
