...
$.GetContextPanel().GetParent().GetParent().GetParent().FindChildTraverse("AghsStatusScepterContainer").GetParent().style["visibility"] = "collapse";
...
{	
	...
	for (let name in data[heroToShow]) {	
		if (name != "heroid") {
			...
			var desc = GameUI.ReplaceDOTAAbilitySpecialValues(name, $.Localize("DOTA_Tooltip_Ability_" + name + "_Description"), rewardPanel);
...
{		
	...
	UpgradesWikiScenePanel.SetPanelEvent("onactivate", () =>
		ShowRewardWiki(Players.GetPlayerSelectedHero( LocalID))		
	);
	...
	for (let hero in kv) {	
		if (hero != "splitscreenplayer") {
			...
			nameplate.SetPanelEvent("onactivate", () =>
				ShowRewardWiki(hero)
			);
...
function ShoWikiButton() {
	GameEvents.SendCustomGameEventToServer("get_heroes_list", {self_hero_name : Players.GetPlayerSelectedHero( LocalID )});
