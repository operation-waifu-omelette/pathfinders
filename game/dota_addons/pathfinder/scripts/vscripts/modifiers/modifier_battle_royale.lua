...
function modifier_battle_royale:OnIntervalThink()
	...
	if hRoom ~= nil then
		...
		if bValidPosition == false or bValidRoom == false and hCurrentRoom ~= "encounter_starting_room" and hCurrentRoom ~= nil then
	...
	if ( GameRules:GetGameTime() - self.flLastTimeInCurrentRoom ) > 8.0 and self:GetParent():IsSummoned() == false and not self:GetParent():HasModifier( "modifier_hero_select" ) and self:GetParent():GetUnitName() ~= "npc_dota_hero_wisp" and not self:GetParent():HasModifier("modifier_dummy") then
