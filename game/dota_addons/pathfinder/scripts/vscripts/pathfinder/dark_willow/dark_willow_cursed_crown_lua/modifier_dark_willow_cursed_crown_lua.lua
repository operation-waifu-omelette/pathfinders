...
function modifier_dark_willow_cursed_crown_lua:OnIntervalThink()
	...
	if IsServer() then
		if self:GetCaster():HasAbility("dark_willow_cursed_crown_lua_thorns") and self:GetCaster():FindAbilityByName("dark_willow_bramble_maze_lua"):IsTrained() then
			self:GetCaster():FindAbilityByName("dark_willow_bramble_maze_lua"):CastBrambles(self:GetParent():GetOrigin())
		end
	end
