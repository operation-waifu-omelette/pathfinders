...
function aghanim_spell_swap:OnSpellStart()
	if IsServer() then
		...
		if hSummonPortals then
			...
			local summonPos = self:GetCaster():GetAbsOrigin() + RandomVector(RandomFloat(200,350))
			CreateModifierThinker( self:GetCaster(), self, "modifier_aghanim_summon_portals_thinker", kv, summonPos, self:GetCaster():GetTeamNumber(), false )
		end
